...
m = 100
x = torch.unsqueeze(torch.linspace(-1, 1, m), dim=1)  # 数据点横坐标
x_test = torch.unsqueeze(torch.linspace(-2, 2, 4*m), dim=1)  # 数据点横坐标
...
y_test = x_test.pow(4) + 0.1 * torch.randn(x_test.size())  # 数据点对应的纵坐标
x_test, y_test = (Variable(x_test), Variable(y_test))  # training set, pytorch handlable
...
for t in range(T):  # 迭代5000次
    ...
    prediction0 = net(x_test)  # 先用网络对所有的x预测一次,是一个数组
    ...
    loss = loss_func(prediction0, y_test)  # 我们计算SGD选出的样本的损失

    optimizer.zero_grad()  # 把net全部的梯度归零
    loss.backward()  # 向后传播
    optimizer.step()

    prediction = net(x_test)  # 更新后的再预测一次
    if t == 0:
    ...
    # print(threshold)
    loss = loss_func(prediction, y_test)  # 计算总损失
...
utils.plot_sgd_gd(T, x_test, y_test, x, y, prediction_table, prediction0_table, selected_table, selectedy_table,q_table, name = f'gd_vs_sgd_wide_deep_{depth}_width_{width}_point')
