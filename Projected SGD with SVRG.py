...
class Net(nn.Module):
    ...
    def calculate_loss_grad_loader(self, dataset, criterion, n_samples):
        """
        Function to compute the full loss and the full gradient
        args : dataset, loss function and number of samples
        return : total loss and full grad norm
    ...
    def projected_SGD_backward(self, criterion, n_epoch, lr):
        """
        ...
        for epoch in range(n_epoch):
            ...
            total_loss_epoch[epoch], _ = \
                previous_net_grad.calculate_loss_grad_loader(train_loader, criterion, n_samples_total)
            ...
            for i_batch in range(n_samples):
                ...
                for data_label in index:
                    ...
                    # Compute approximate full grad by SVRG
                    inputs, labels = next(data_iter, (None, None))
                    if inputs == None:
                        data_iter = iter(train_loader)
                    ...
                    for param1, param2, param3 in zip(self.parameters(), previous_net_sgd.parameters(),
                        ...
                        full_grad.append(param1.grad.data - param2.grad.data + (1. / n_samples_total) * param3.grad.data)
...
total_loss_epoch_newalg = \
    net_newalg.projected_SGD_backward(criterion, EPOCHS, Learning_Rate_New)
