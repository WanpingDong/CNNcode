...
def calFrac(device):
    ...
    for batch_idx, (data, target) in enumerate(train_loader1):
        data, target = data.to(device), target.to(device)  # CPU转GPU
        output = net2(data)
        arr1[batch_idx] = math.log(math.fabs((loss_func(output, target) / arr1[batch_idx])/(loss1 / loss0)))


    for batch_idx, (data, target) in enumerate(train_loader2):
        data, target = data.to(device), target.to(device)  # CPU转GPU
        output = net2(data)
        arr2[batch_idx] = math.log(math.fabs((loss_func(output, target) / arr2[batch_idx])/(loss1 / loss0)))


    plt.hist(arr1, bins=3, color='gold', alpha=0.7,
             label='the relative similarity of {} and {}'.format(data_label0, data_label1))
    plt.hist(arr2, bins=3, color='salmon', alpha=0.7,
